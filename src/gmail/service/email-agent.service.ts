...
export class EmailAgentService {
  ...
  async connectGmail(userId: string, accessToken: string, refreshToken: string, expiryDate: Date) {
    ...
    // Test the connection
    const isConnected = await this.gmailService.testConnection(userId);
  ...
  async getGmailStatus(userId: string) {
    ...
    // Test if connection is still valid
    const isActive = await this.gmailService.testConnection(userId);
  ...
  async runEmailDigest(userId: string) {
    this.logger.log(`Running email digest for user: ${userId}`);
    ...
    const lastChecked = lastDigest?.createdAt || new Date(Date.now() - 24 * 60 * 60 * 1000);

    // Fetch unread emails
    const unreadEmails = await this.gmailService.getUnreadEmails(userId, lastChecked);
    this.logger.log(`Found ${unreadEmails.length} unread emails`);
    ...
    for (const email of unreadEmails) {
      try {
        ...
        processedEmailIds.push(email.id as string);
      } catch (error) {
        this.logger.error(`Failed to analyze email ${email.id}:`, error);
      }
    }
    ...
    for (const emailId of processedEmailIds) {
      try {
        await this.gmailService.markAsRead(userId, emailId);
      } catch (error) {
        this.logger.error(`Failed to mark email ${emailId} as read:`, error);
      }
    }
